...
function createData(assets, available, variable_apy, stable_apy, reserve_address) {
    return {
        assets,
        available,
        variable_apy,
            ...
            <TableRow className={ market.assets.isReserveEnabled ? "blue-border" : ""}>
                <TableCell scope="row">
                    <img
                        ...
                        src={tokenSymbol(market.assets.symbol)}
                        alt="Assets"
                    />
                    {market.assets.name}
                </TableCell>
                <TableCell>
                    {
                        account.availableBorrows === 0
                            ? "--"
                            : ((currencyType === 1 ? "$ " : "") +
                            beautyNumber((account.availableBorrows / market.available.assetsPriceETH) * (market.available.assetsPrice ** currencyType)))
                    }
                </TableCell>
                <TableCell>
                    {market.variable_apy === 0 ? "--" : market.variable_apy + "%"}
                </TableCell>
                ...
                <TableCell className="mobile-hide">
                    {market.stable_apy === 0 ? "--" : market.stable_apy + "%"}
                </TableCell>

                <TableCell className="mobile-hide">
                    <Link
                        to={`/borrowDetail/${market.reserve_address}`}
                    ...
                    <Collapse in={open} timeout="auto" unmountOnExit>
                        <Box margin={1}>
                            <Table className="border-none" aria-label="purchases">
                                <TableHead>
                                    <TableRow>
                                        <TableCell>Stable APY</TableCell>
                                        <TableCell>{market.row_inner_values.stable_apy}%</TableCell>
                                    </TableRow>
                                    <TableRow>
                                        <TableCell colSpan={2}>
        ...
        assets.filter(asset => asset.isActive).map(
            (value, index) => {
                rows.push(createData(
                    ...
                    {
                    assetsPriceETH: value.assetsPriceETH,
                                                ...
                                                <Table className="border-none" aria-label="simple table">
                                                    <TableHead>
                                                        <TableRow >
                                                            ...
                                                            <StyledTableCell>Available for you</StyledTableCell>
                                                            <StyledTableCell>Variable APY</StyledTableCell>
                                                            <StyledTableCell className="web-hide" />
                                                            <StyledTableCell className="mobile-hide">Stable APY</StyledTableCell>
                                                            <StyledTableCell className="mobile-hide" />
                                                        </TableRow>
                                                    </TableHead>
                                                    <TableBody>
                                                        {rows.map((row,index) => (
                                                            <Row row={row} key={index} currencyType={currencyType} account={account}/>
                                                        ))}
